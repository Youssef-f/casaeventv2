  ...
  useEffect(() => {
    ...
    events.forEach((event) => {
          ...
          <div class="space-y-2 mb-3">
            ...
            <div class="flex items-center gap-2">
              <div class="flex items-center gap-1">
                <span class="text-sm font-bold text-gray-900">${event.rating.toFixed(
                  1
                )}</span>
                <div class="flex">
                  ${Array.from(
                    ...
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="${
                      i < Math.floor(event.rating) ? "#FCD34D" : "none"
      ...
      marker.on("popupopen", () => {
        ...
        if (popupEl) {
          const buyBtn = popupEl.querySelector("button");
          if (buyBtn) {
    ...
    if (events.length === 0) {
      ...
      defaultLocations.forEach((location) => {
        L.marker(location.position, { icon: eventIcon })
          .addTo(map)
          .bindPopup(
            `<div class="p-2"><strong>${location.name}</strong><br/>Événements disponibles</div>`
          );
      });
    }
